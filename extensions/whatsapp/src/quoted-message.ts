...
function makeCacheKey(remoteJid: string, messageId: string): string {
...
export function cacheInboundMessageMeta(
  ...
  cache.set(makeCacheKey(remoteJid, messageId), { ...meta, ts: Date.now() });
}
...
export function lookupInboundMessageMeta(
  ...
  const entry = cache.get(makeCacheKey(remoteJid, messageId));
  if (!entry) {
    return undefined;
  }
  if (Date.now() - entry.ts > CACHE_TTL_MS) {
    cache.delete(makeCacheKey(remoteJid, messageId));
    return undefined;
  }
